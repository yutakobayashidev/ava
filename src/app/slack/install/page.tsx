...
export default async function SlackInstallPage({
    ...
    const statusMessage = (() => {
        if (params.installed === "1") {
            return `Slackボットをインストールしました (${params.team ?? "workspace"})`;
        }
        if (params.error) {
                ...
                <div className="border-t border-gray-200 pt-4 space-y-2">
                    ...
                    {workspace ? (
                        <div className="text-sm text-gray-700 space-y-1">
                            ...
                            <p>bot token: {workspace.botAccessToken ? "保存済み" : "未保存"}</p>
                        </div>
                    ) : (
                        <p className="text-sm text-gray-600">まだSlackワークスペースは連携されていません。</p>
                    )}
                </div>
            </div>
        </main>
