...
<Tabs items={['Claude Code', 'Cursor']} persist groupId="coding-agent">
  <Tab value="Claude Code">
    ...
    **スキルをインストールすると:**
    ...
    もちろん、自分でClaude Skillsを作成したり、CLAUDE.mdに独自のルールを追記することで、報告の仕方をカスタマイズすることもできます。

    <Accordions type="single">
      <Accordion title="Claude Skillsとは？">
        **Claude Skills** は、Claude Codeに特定の知識や手順を教えるための仕組みです。

        Skillsを使うことで：
        - 特定のツールの使い方をClaude Codeに教える
        - プロジェクト固有のルールやワークフローを定義する
        - 繰り返し使う手順を自動化する

        などが可能になります。

        **Avaスキルの役割:**
        - AvaのMCPツールの使い方のベストプラクティスを提供
        - プライバシーに配慮した報告方法を自動的に適用
        - 報告のタイミングや粒度のデフォルト設定を提供

        詳しくは [Claude Skills公式ドキュメント](https://support.claude.com/ja/articles/12512176-%E3%82%B9%E3%82%AD%E3%83%AB%E3%81%A8%E3%81%AF%E4%BD%95%E3%81%A7%E3%81%99%E3%81%8B) をご覧ください。
      </Accordion>
    </Accordions>
  </Tab>
