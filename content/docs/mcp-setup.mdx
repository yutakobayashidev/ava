...
<Tabs items={['Claude Code', 'Cursor', 'Codex']} persist groupId="coding-agent">
  ...
  <Tab value="Codex">
    ...
    Codex CLI と IDE 拡張は同じ設定ファイル（`~/.codex/config.toml`）を共有します。

    ### 2. RMCP クライアントを有効化（OAuth 用）

    AvaのMCPサーバーはOAuth 2.1で認可を行います。`[features]` セクションに以下を追加し、RMCPクライアントを有効化してください。
    ...
    CLI から設定する場合は `codex mcp --help` を参照してください。

    ### 4. Codex を再起動して接続

    Codex を再起動し、ターミナルで `codex mcp login ava` を実行してブラウザ認可を許可してください。

    ### 5. 接続確認

    `codex` のチャットで「利用可能な MCP ツールを教えて」と入力し、`start_task` などの Ava ツールが表示されれば完了です。
